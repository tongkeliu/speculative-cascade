...
def args_parser():
    ...
    parser.add_argument(
        "--alpha", type=float, default=0.2, help="cost of switching to target model"
    )
    parser.add_argument(
        "--approx_model",type=str,default="qwen3-0.6b",
        help="a small model to approximate large one",
        choices=["qwen3-0.6b"]
    )
    parser.add_argument(
        "--target_model", type=str, default="qwen3-8b", help="the target model",
        choices=["qwen3-8b, qwen3-4b"]
    )
    parser.add_argument(
        "--def_rule", type=str, default="chow", help="deferral rule for cascade"
    )
    parser.add_argument(
        "--spec_def_rule", type=str, default='token', help="select the deferral rule",
        choices=["speculative", "chow", "optimal", "token"]
    )
...
def inference_speed(fn, sample_num, logger, method, sampler):
    token_sum = 0
    with contexttimer.Timer() as t:
        for i in range(sample_num):
            res = fn()
            logger.info("model answer:{}".format(sampler.tokenizer.decode(res[0])))
...
def deferral_rule(rule, q, p, alpha, token):
    if rule == 'speculative':
        return torch.tensor([True], device=p.device)
    elif rule == 'chow':
        ...
        q_max = torch.max(torch.softmax(q, dim=1))
        p_max = torch.max(torch.softmax(p, dim=1))
        D_tv = torch.max(torch.softmax(p - q, dim=1))
        return q_max < p_max - alpha * D_tv
    elif rule == 'token':
        q_max = torch.max(torch.softmax(q, dim=1))
        p_max = torch.max(torch.softmax(p, dim=1))
        q_v, p_v = q[:, token], p[:, token]
        # version 1
        return q_v < p_max - alpha
        # # version 2
        # return p_v < p_max - alpha
        # # version 3
        # return p_v < p_max * (1 - alpha)


def distribution(deferral, q_v, p_v):
    return ~deferral * q_v + deferral * p_v