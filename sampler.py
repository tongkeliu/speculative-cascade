...
class Sampler():
    ...
    def spec_sample(self, rule):
        ...
        while n < self.args.max_length:
            ...
            for i in range(self.args.gamma):
                ...
                r = deferral_rule(rule, ap_prob_history[:, n+i, :],
                                tg_prob_history[:, n+i, :], self.args.alpha, token)
                p, q = tg_prob_history[:,n+i,:], ap_prob_history[:,n+i,:]
                pi = distribution(r, q[:, token], p[:, token])
                # reject condition
                randnum = torch.rand((1, 1), device=self.args.device)
                if randnum < 1 - pi / q[:, token]:
                    probs = max_fn(distribution(r, q, p) - q)
        ...
        self.logger.info(f"[{rule}] reject_cnt:{reject_cnt}")
        return result_ids
    ...
    def cascade(self):
        ...
        for i in range(self.args.max_length):
            ...
            r = deferral_rule(self.args.def_rule, ap_output.logits[:, -1, :],
                             None, self.args.alpha, None)
            if r:
        ...
        ap_past_key_values.reset()
        tg_past_key_values.reset()
        self.logger.info(f"switch_cnt:{switch_cnt}")
        return result_ids
    
    @torch.no_grad()
    def speculative(self):
        return self.spec_sample("speculative")
    
    @torch.no_grad()
    def spec_cascade(self):
        return self.spec_sample(self.args.spec_def_rule)